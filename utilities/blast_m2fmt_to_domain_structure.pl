...
my $usage = "usage: $0 domain_tblastn_m2fmt [DP_flag=0] [protToCDScoordFlag=0]\n\n";
...
while (<$fh>) {
	chomp;
	my @x = split (/\t/);
	my ($seq_acc, $domain_instance, $evalue, $score, $orient, $lend, $rend) = ($x[0], $x[1], $x[2], $x[4], $x[16], $x[17], $x[18]);
